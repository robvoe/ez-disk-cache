...
def disk_cache(
        ...
        max_cache_root_size_mb: Optional[float] = None, max_cache_elements: Optional[int] = None,
    ...
    @param max_cache_elements: Maximum number of cache instances within the cache root folder. If None, there is no
    ...
    """
    assert max_cache_elements is None or max_cache_elements > 0, "Value greater than 0 expected!"
    ...
    if max_cache_root_size_mb is None and max_cache_elements is None and ENABLE_WARNINGS is True:
    ...
    def decorating_function(user_function):
        wrapper = _disk_cache_wrapper(
            ...
            max_cache_root_size_mb=max_cache_root_size_mb, max_cache_elements=max_cache_elements,
...
def _disk_cache_wrapper(user_function, cache_root_folder: Path, max_cache_root_size_mb: Optional[float],
                        max_cache_elements: Optional[int], iterable_loading_strategy: str,
    ...
    def wrapper(*args, **kwargs) -> Any:
        ...
        if _cache_instance_path is not None:
            ...
            _cached_data = _load_cache_instance(cache_instance_path=_cache_instance_path,
        ...
        _cleanup(cache_root_folder=cache_root_folder, max_cache_root_size_mb=max_cache_root_size_mb,
                 max_cache_elements=max_cache_elements, youngest_cache_instance_path=_cache_instance_path)

        # Deliver the cache now
        _cached_data = _load_cache_instance(cache_instance_path=_cache_instance_path,
                                            iterable_loading_strategy=iterable_loading_strategy)
        return _cached_data
...
def _load_cache_instance(cache_instance_path: Path, iterable_loading_strategy: str) -> Any:
    """Loads a cache instance. Distinguishes between simple cache "blobs" and iterables (shelves)."""
    ...
    try:
        with open(file=cache_instance_path / _LAST_USAGE_FILENAME, mode="w") as file:
            file.write("Nothing interesting in here.")
    except IOError:
    ...
    _is_iterable = any(filter(lambda path: path.name == _ITERABLE_CACHE_VALUE_FILENAME, cache_instance_path.iterdir()))
    if _is_iterable is True:
...
class LazyList:
    ...
    def __str__(self) -> str:
        if self._iterable_loading_strategy == "lazy-load-keep":
            return f"[{len(self)} lazy objects, of which {sum(self._storage_data_loaded)} are held in RAM]"
        return f"[{len(self)} lazy objects]"
...
def _cleanup(cache_root_folder: Path, max_cache_root_size_mb: Optional[float], max_cache_elements: Optional[int],
    ...
    def _get_instance_usage_time(_folder: Path) -> float:
        _element_of_interest = (_folder/_LAST_USAGE_FILENAME) if (_folder/_LAST_USAGE_FILENAME).is_file() else _folder
        return _element_of_interest.stat().st_mtime
    ...
    if _cleanup_necessary is True:
        ...
        _n_deleted = 0
        _instance_usage_time__paths = {_get_instance_usage_time(path): path for path in cache_root_folder.iterdir() if
        ...
        assert _n_instances >= 1  # "Fear-assert" to make sure the above code works correctly
